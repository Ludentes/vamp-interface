...
### 2026-04-28 — Solver C as partial-Fisher estimator; squint feasibility check planned

Glasses slider thread converged (`v8` r=32, α=1, peak=20, lr=1.25e-4 const, ~2450 steps; in-dist Spearman ρ=+0.766, held-out +0.792, beats v7-1200 on both). Pivot to eye-squint as the next axis. Reasoning: squint bundles in the text encoder with smile/age/lighting/scrutiny — Path A's structural weakness on bundled axes ([solvers-taxonomy](../2026-04-26-solvers-taxonomy-and-next-steps.md) lines 185–196) becomes load-bearing where it didn't for glasses. Solver C (intra-cluster pair selection) formalized as a finite-difference estimator of the partial Fisher information `I_{θ|η} = I_θθ − I_θη I_ηη^{−1} I_ηθ`; the doc's hand-set "Δintended large, Δconfound small" rule is the Schur-complement criterion in disguise, with W = Σ_η^{−1}. Buys: parameter-free pair scoring, calibrated stopping criterion, principled Path-B-vs-Path-C comparison (both are estimators of the same I_{θ|η}). Next: feasibility script on `sample_index.parquet` (eyeSquintLeft+Right vs smile/age/lighting/head-pose/identity-drift confounds, within-demographic-cell pairs, J(i,j)=(Δθ)²/‖Δη‖_W²). See [2026-04-28-solver-c-partial-fisher.md](../2026-04-28-solver-c-partial-fisher.md).
