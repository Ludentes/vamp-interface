...
The module also exposes `local_jacobian(verts, u) → (N,3,3)`: the per-vertex Jacobian of the full field, obtained via autograd (`torch.func.jacrev` vmapped, or analytic since the field is structured). This is what rescales the blendshape basis — replacing the hand-derived `diag(s_r, s_y, s_r)` in the current code with the *exact* Jacobian of whatever field the fit lands on.
