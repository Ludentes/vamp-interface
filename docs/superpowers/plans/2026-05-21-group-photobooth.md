...
**Files:**
- Create: `scripts/build_group_workflows.py` (programmatic builder)
- Create: `comfyui/workflows/group_sam2_mask.api.json` (generated by builder)
- Create: `comfyui/workflows/group_cutout.api.json` (generated by builder)
...
Both workflows are constructed programmatically in `scripts/build_group_workflows.py` (same pattern as `scripts/build_inpaint_workflow.py`). Re-run the builder whenever the graphs change; the produced `.api.json` files are committed so the runtime path is plain `json.load`.

- [ ] **Step 1: Implement the workflow builder**

`scripts/build_group_workflows.py`:
```python
...
def build_sam2_mask_workflow() -> dict:
    ...
    Output node id "5" returns the mask as an RGB image (white = mask).
    """
    return {
...
def build_cutout_workflow() -> dict:
    ...
    Output node id "3" returns the cutout as an RGBA PNG.
    """
    return {
        "1": {"class_type": "LoadImage",
              "inputs": {"image": "$$IMAGE"}},
...
- [ ] **Step 2: Generate the workflow JSONs**

```bash
uv run --no-project python scripts/build_group_workflows.py
```

Expected: `comfyui/workflows/group_sam2_mask.api.json` and `comfyui/workflows/group_cutout.api.json` created. Verify each contains the `$$IMAGE` / `$$BBOX_JSON` literal strings.

**Note on node names/inputs:** The exact `class_type` strings and input keys above are taken from the `kijai/ComfyUI-segment-anything-2` and `1038lab/ComfyUI-RMBG` READMEs. If the live ComfyUI rejects either workflow with a "node not found" or "missing input" error during Step 7 (live smoke), open the ComfyUI web UI on the same host, drop the node onto a canvas, inspect the inputs panel, and patch the builder. Re-run the builder, re-test.
