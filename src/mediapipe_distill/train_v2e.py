"""Train the U-Net BlendshapeLandmarkStudentUNet (variant `bs_v2e`).

Same dataset and combined loss as v2d. The only differences vs train_v2d.py
are the student class and (optionally) a longer schedule for the overnight
3-4h run.
"""
...
def evaluate(model: BlendshapeLandmarkStudentUNet, loader: DataLoader, device: torch.device):
    model.eval()
    bs_preds, bs_targets = [], []
...
def main():
    ...
    p.add_argument("--variant", default="bs_v2e")
    ...
    print(f"variant={args.variant} device={device} epochs={args.epochs} "
          f"λ_lmk={args.lambda_lmk} λ_bs={args.lambda_bs}")
    ...
    model = BlendshapeLandmarkStudentUNet(args.variant).to(device)
    ...
    for epoch in range(args.epochs):
        ...
        torch.save(ck_payload, args.out_dir / f"epoch_{epoch:03d}.pt")
