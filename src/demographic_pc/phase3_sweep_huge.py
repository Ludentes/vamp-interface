"""Phase 3 huge-scale sweep with renorm ON. Push past 100 to see if the
...
OUT = ROOT / "output/demographic_pc/phase3_sweep_huge"
...
async def run() -> None:
    ...
    async with ComfyClient() as client:
        for base_name, base_prompt in BASES:
            for s in SCALES:
                stem = f"{base_name}_s{TEST_SEED}_x{s:+.1f}"
                dest = OUT / f"{stem}.png"
                if dest.exists():
                    print(f"[skip] {stem}")
                    continue
                print(f"[render] {stem}")
                await client.generate(wf(TEST_SEED, f"p3huge_{stem}", base_prompt, s), dest)
    print(f"[done] → {OUT}")
