"""Phase 3 wide-scale sweep: push the cached full-tensor replay past 1.5 to
find the actual edit-vs-collapse regime. Same test seed (777, unseen)."""
...
OUT = ROOT / "output/demographic_pc/phase3_scale_sweep"
...
def wf(seed: int, prefix: str, base_prompt: str, scale: float) -> dict:
    ...
    w["22"] = {
        ...
        "inputs": {
            ...
            "scale": float(scale), "start_percent": 0.15, "end_percent": 1.0, "verbose": False,
        },
    }
        ...
        for base_name, base_prompt in BASES:
            for s in SCALES:
                ...
                if dest.exists():
                    ...
                    continue
                print(f"[render] {stem}")
                await client.generate(wf(TEST_SEED, f"p3sweep_{stem}", base_prompt, s), dest)
    print(f"[done] → {OUT}")
