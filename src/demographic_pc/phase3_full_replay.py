"""Phase 3: test-render the Phase 2 seed-averaged full-tensor library via
FluxSpaceDirectionInjectFull.

Decision gate:
  - same base (elderly_latin_m), new seed 777: does smile transfer?
  - different base (young_european_f), seed 777: does it cross-transfer?
  - scale sweep [0, 0.5, 1.0, 1.5] at each

If same-base-new-seed shows smile but cross-base fails → ceiling is latent-
specificity, not token-structure.
If both fail → option B falsified, prompt-pair is the only path.
If both succeed → option B is viable, scale to library.

Run:
  uv run python -m src.demographic_pc.phase3_full_replay --run
"""
...
OUT = ROOT / "output/demographic_pc/phase3_full_replay"

BASES = [
    ("elderly_latin_m", CALIBRATION_PROMPTS[5][1]),   # same base as capture
    ("young_european_f", CALIBRATION_PROMPTS[8][1]),  # cross-base
]
TEST_SEED = 777  # NOT one of {2026, 4242, 1337} used in capture
...
def replay_workflow(seed: int, prefix: str, base_prompt: str, scale: float) -> dict:
    wf = _base_workflow()
        ...
        for base_name, base_prompt in BASES:
            for s in SCALES:
                ...
                await client.generate(
                    replay_workflow(TEST_SEED, f"phase3_replay_{stem}", base_prompt, s),
                    dest,
                )
    print(f"[done] → {OUT}")
...
def main() -> None:
    ...
    if args.run:
        ...
        print(f"Would render {len(BASES)} bases × {len(SCALES)} scales = "
              f"{len(BASES)*len(SCALES)} images")
        print(f"  library: {LIB} ({LIB.stat().st_size/1e6:.0f} MB)")
        print(f"  test seed: {TEST_SEED} (not in capture seeds)")
        print(f"  → {OUT}")
